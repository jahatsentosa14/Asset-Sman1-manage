...
export function LoanApprovalCard({
  loan,
  onProcessed,
}: {
        ...
        <RejectReasonInput
          ...
          onConfirm={(reason) => {
            ...
            startTransition(async () => {
              const result = await rejectLoanAction(loan.id, reason);
              if (result.error) { setError(result.error); toast.error(result.error); }
              else { onProcessed?.(); toast.success('Peminjaman ditolak.'); }
            });
          }}
        />
      ) : (
          ...
          <Button
            ...
            onClick={() => {
              ...
              startTransition(async () => {
                ...
                if (result.error) { setError(result.error); toast.error(result.error); }
                else { onProcessed?.(); toast.success(`Peminjaman ${loan.profiles?.full_name ?? ''} disetujui.`); }
              });
            }}
          >
