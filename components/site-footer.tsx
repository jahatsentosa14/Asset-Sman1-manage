...
export function SiteFooter() {
  return (
    <footer className="mt-auto border-t border-[#1A123B]/10 bg-[#1A123B] px-4 py-8 text-white">
      <div className="mx-auto max-w-6xl space-y-3 text-center">
        ...
        <nav className="flex flex-wrap justify-center gap-x-5 gap-y-2 text-xs text-white/60">
          {LEGAL_LINKS.map((link) => (
            <Link key={link.href} href={link.href} className="transition-colors duration-300 hover:text-[#FDBB2D] hover:underline">{link.label}</Link>
          ))}
        </nav>
      </div>
    </footer>
