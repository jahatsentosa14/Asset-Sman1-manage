  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-4">
        <div><h1 className="text-2xl font-black tracking-tight text-[#000047]">{greeting.title}</h1><p className="text-muted-foreground">{greeting.subtitle}</p></div>
      ...
      {grouped.length > 0 && <FadeIn className="glass-panel rounded-3xl p-5 sm:p-7"><div className="mb-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#FFDB58]">School Information</p><h2 className="mt-1 text-xl font-black text-[#000047]">Informasi Sekolah</h2><p className="text-sm text-muted-foreground">Denah, tata ruang, luas tanah, dan daftar ruangan.</p></div><InformationGallery grouped={grouped} /></FadeIn>}

      {activeLoans.length > 0 && (
        <FadeIn className="space-y-3"><h2 className="text-lg font-semibold text-[#000047]">Pinjaman Aktif Anda</h2><div className="space-y-3">
          {activeLoans.map((loan) => <div key={loan.id} className="glass-panel flex items-center justify-between rounded-2xl p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-[#FFDB58]">
